...
function drawRoute(cityPath) {
  ...
  for (var i = 0; i < cityPath.length; i++) {
    ...
    var distance = CalculateDistance(startCoor, endCoor);
    var midPoint = [(startCoor[0] + endCoor[0]) / 2, (startCoor[1] + endCoor[1]) / 2];
    ...
    ctx.font = "10px Arial";
    ctx.fillStyle = "#000000";
    ctx.fillText(Math.round(distance), midPoint[0], midPoint[1]);
  }

  // Draw the nodes after drawing the route
  for (var i = 0; i < cityCoordinates.length; i++) {
    var xPos = cityCoordinates[i][0];
    var yPos = cityCoordinates[i][1];
